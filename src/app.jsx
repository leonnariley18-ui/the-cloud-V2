    ...
    <GlassCard>
      ...
      <div style={{position:"relative"}}><input value={cop.name} onChange={e=>setCop({...cop,name:e.target.value,existingStrainId:null})} onFocus={()=>setShowSugg(true)} placeholder="enter strain name..." style={{width:"100%",boxSizing:"border-box",background:"rgba(240,235,225,0.06)",borderRadius:8,padding:"10px 14px",fontSize:15,color:"#F0EBE1",border:"0.5px solid rgba(240,235,225,0.1)",fontFamily:"inherit",outline:"none",marginBottom:cop.existingStrainId?4:20}}/>
        {showSugg&&(nameSuggs.length>0||legacySuggs.length>0)&&!cop.existingStrainId&&<div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:10,background:"#2C2420",border:"0.5px solid rgba(240,235,225,0.15)",borderRadius:8,marginTop:2,maxHeight:160,overflowY:"auto"}}>
          ...
          {legacySuggs.map(l=><button key={l.id} onClick={()=>{setCop({...cop,name:l.name,type:l.type||cop.type});setShowSugg(false);}} style={{display:"flex",width:"100%",padding:"10px 14px",fontSize:13,color:"#F0EBE1",background:"transparent",border:"none",textAlign:"left",cursor:"pointer",fontFamily:"inherit",alignItems:"center",gap:8}}><div style={{width:4,height:24,borderRadius:2,background:l.type?typeColor(l.type):"#4A3E38"}}/><span style={{fontWeight:500}}>{l.name}</span><span style={{fontSize:11,color:"rgba(240,235,225,0.4)"}}>legacy</span></button>)}
        </div>}
      </div>
      ...
      {cop.source==="Dispensary"&&<><SubToggle options={["Bag","Jar"]} value={cop.container} onChange={v=>setCop({...cop,container:v})} color={P.terracotta} label="packaging:"/>
        ...
        <input value={cop.brand} onChange={e=>setCop({...cop,brand:e.target.value})} placeholder="e.g. cookies, jungle boys..." style={{width:"100%",boxSizing:"border-box",background:"rgba(240,235,225,0.06)",borderRadius:8,padding:"10px 14px",fontSize:14,color:"#F0EBE1",border:"0.5px solid rgba(240,235,225,0.1)",fontFamily:"inherit",outline:"none"}}/></div>
      </>}<div style={{marginBottom:20}}/>
      ...
      {!cop.existingStrainId&&<><label style={{fontSize:12,fontWeight:500,color:"rgba(240,235,225,0.5)",display:"block",marginBottom:6}}>parent strains <span style={{fontWeight:400,fontStyle:"italic"}}>(optional)</span></label>
        {!cop.unknownLineage&&<div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:8}}><input value={cop.parent1} onChange={e=>setCop({...cop,parent1:e.target.value})} placeholder="parent 1" style={{width:"100%",boxSizing:"border-box",background:"rgba(240,235,225,0.06)",borderRadius:8,padding:"10px 14px",fontSize:14,color:"#F0EBE1",border:"0.5px solid rgba(240,235,225,0.1)",fontFamily:"inherit",outline:"none"}}/><input value={cop.parent2} onChange={e=>setCop({...cop,parent2:e.target.value})} placeholder="parent 2" style={{width:"100%",boxSizing:"border-box",background:"rgba(240,235,225,0.06)",borderRadius:8,padding:"10px 14px",fontSize:14,color:"#F0EBE1",border:"0.5px solid rgba(240,235,225,0.1)",fontFamily:"inherit",outline:"none"}}/></div>}
      ...
      <textarea value={cop.notes} onChange={e=>setCop({...cop,notes:e.target.value})} placeholder="smell, look, expectations..." rows={2} style={{width:"100%",boxSizing:"border-box",background:"rgba(240,235,225,0.06)",borderRadius:8,padding:"10px 14px",fontSize:14,color:"#F0EBE1",border:"0.5px solid rgba(240,235,225,0.1)",fontFamily:"inherit",outline:"none",resize:"vertical",marginBottom:20}}/>
  ...
  return(<Wrapper>
    ...
    {onHand.length>0&&<><StashLabel>on hand</StashLabel>{onHand.map(o=>{const s=strains.find(ss=>ss.id===o.strainId);const intent=s?.intent;return(<SolidCard key={o.copId} style={{marginBottom:8,border:`1px solid rgba(91,138,114,0.3)`}}>
      ...
      {inlineCard?.copId===o.copId&&inlineCard?.mode==="experience"&&<div style={{marginTop:10,paddingTop:10,borderTop:"0.5px solid rgba(240,235,225,0.08)"}}>
        <textarea value={inlineText} onChange={e=>setInlineText(e.target.value)} placeholder="how was it? what did you do, feel, notice..." rows={4} style={{width:"100%",background:"rgba(240,235,225,0.06)",border:"0.5px solid rgba(240,235,225,0.12)",borderRadius:8,padding:10,fontSize:13,color:"#F0EBE1",fontFamily:"inherit",resize:"none",boxSizing:"border-box",outline:"none"}}/>
        <div style={{display:"flex",gap:6,marginTop:6}}>
          <button onClick={closeInline} style={{flex:1,padding:8,borderRadius:8,fontSize:12,background:"transparent",color:"rgba(240,235,225,0.4)",border:"0.5px solid rgba(240,235,225,0.1)",cursor:"pointer",fontFamily:"inherit"}}>cancel</button>
  ...
  return(<div style={{background:typeTheme.bg,minHeight:"100vh",color:typeTheme.text}}><div style={{padding:"72px 24px 60px",maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{flex:1}}>
        ...
        {editingParents&&<div style={{marginTop:6}}>
          {!unknownLineage&&<div style={{display:"flex",gap:6,marginBottom:6}}>
            <input value={p1} onChange={e=>setP1(e.target.value)} placeholder="parent 1" style={{flex:1,background:typeTheme.cardBg,borderRadius:6,padding:"6px 10px",fontSize:12,color:typeTheme.text,border:`0.5px solid ${typeTheme.accent}40`,fontFamily:"inherit",outline:"none"}}/>
            <input value={p2} onChange={e=>setP2(e.target.value)} placeholder="parent 2" style={{flex:1,background:typeTheme.cardBg,borderRadius:6,padding:"6px 10px",fontSize:12,color:typeTheme.text,border:`0.5px solid ${typeTheme.accent}40`,fontFamily:"inherit",outline:"none"}}/>
          </div>}
    ...
    {tab==="notes"&&<div>
      ...
      {mixMode==="note"&&tab==="notes"&&<div style={{background:typeTheme.cardBg,borderRadius:12,padding:16,border:`0.5px solid ${typeTheme.cardBorder}`,marginBottom:12}}>
        ...
        <textarea value={updateNote} onChange={e=>setUpdateNote(e.target.value)} placeholder="what\u2019s on your mind..." rows={3} style={{width:"100%",boxSizing:"border-box",background:"transparent",borderRadius:8,padding:"10px 14px",fontSize:14,color:typeTheme.text,border:`0.5px solid ${typeTheme.cardBorder}`,fontFamily:"inherit",outline:"none",resize:"vertical",marginBottom:12}}/>
    ...
    {tab==="mixes"&&<div>
      ...
      {mixMode==="mix"&&<div style={{background:"rgba(139,109,139,0.08)",borderRadius:12,padding:16,border:"0.5px solid rgba(139,109,139,0.15)",marginBottom:16}}>
        ...
        {mixWith&&<>
          ...
          <textarea value={mixSess.notes} onChange={e=>setMixSess({...mixSess,notes:e.target.value})} placeholder="how'd the combo play together..." rows={2} style={{width:"100%",boxSizing:"border-box",background:"transparent",borderRadius:8,padding:"10px 14px",fontSize:14,color:typeTheme.text,border:"0.5px solid rgba(139,109,139,0.15)",fontFamily:"inherit",outline:"none",resize:"vertical",marginBottom:12}}/>
          <div style={{display:"flex",gap:6}}>
